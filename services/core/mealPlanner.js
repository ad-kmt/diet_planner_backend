...
const getMeals = async (userId)=>{
    ...
    bMeals.forEach(b => {
      lMeals.forEach(l => {
        dMeals.forEach(d => {
          const pErr = Math.abs((b.nutriValues.protein + l.nutriValues.protein + d.nutriValues.protein)-dpr);
          const fErr = Math.abs((b.nutriValues.fat + l.nutriValues.fat + d.nutriValues.fat)-dfr);
          const cErr = Math.abs((b.nutriValues.carb + l.nutriValues.carb + d.nutriValues.carb)-dcr);
